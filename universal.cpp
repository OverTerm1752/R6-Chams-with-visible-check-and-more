...
void __stdcall hookD3D11DrawIndexedInstancedIndirect(ID3D11DeviceContext* pContext, ID3D11Buffer* pBufferForArgs, UINT AlignedByteOffsetForArgs)
{
	...
	if (texRed == nullptr) {
		...
		DXGI_FORMAT format = DXGI_FORMAT_R32G32B32A32_FLOAT; //format
		const float col12 =  0xff00ff00 ;
		D3D11_SUBRESOURCE_DATA initData = { &col12, sizeof(uint32_t), 0 };
...
HRESULT __stdcall hookD3D11Present(IDXGISwapChain* pSwapChain, UINT SyncInterval, UINT Flags)
{
	...
	if (showmenu) {
		...
		ImGui::TextColored(ImGui::ColorConvertU32ToFloat4(ImColor(red, green, blue)), ("Pluto"));
		...
		if (tabb == 1)
		{
			...
			ImGui::Spacing();
			ImGui::Spacing(); 
			ImGui::SliderFloat("Aimbot Fov", &AimFOV, 1.0, 500.0);
			ImGui::Spacing();
			ImGui::Spacing(); 
			ImGui::SliderFloat("Aimbot Smooth", &aimbotsmooth, 1.0, 10.0);
		...
		if (tabb == 2)
		{
			...
			ImGui::Spacing();
			ImGui::Spacing();
			ImGui::Checkbox("Germany crosshair", &nazi2);
			ImGui::Spacing();
			ImGui::Spacing();
	...
	if (watermark)
	{
		...
		ImGui::GetOverlayDrawList()->AddText(ImVec2(22, 24), ImColor(255, 255, 255), ("Made by overhead"));
